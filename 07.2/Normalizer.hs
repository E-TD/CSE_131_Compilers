...
{-
anf _i (GetItem _e1 f _l) = (_i', GetItem _e1' f _l)
  where
    (_i', _e1')           = anf _i _e1
-}
anf _i (GetItem _e1 _f _l) = (_i', stitch bs' (GetItem _e1' _f _l))
  where
    (_i' , bs' , _e1')       = imm _i  _e1

anf _i (App _e _es _l)    = (i', stitch bs (App v vs _l))
  where
  (i', bs, v:vs)        = imms _i (_e:_es)

anf _i (Lam _xs _e _l)    = (_i', Lam _xs _e' _l)
  where
  (_i', _e')            = anf _i _e

anf _i (Fun _f _t _xs _e _l) = (_i', Fun _f _t _xs _e' _l)
  where
    (_i', _e')            = anf _i _e
...
imm _i (Tuple _e1 _e2 _l) = (_i'', bs', mkId x _l)
  where
    (_i', bs, [v1, v2])	    = imms _i [_e1, _e2]
    (_i'', x)		            = fresh _l _i'
    bs' 		                = (x, (Tuple v1 v2 _l, _l)) : bs

--imm _i (GetItem _e1 _f _l) = error "TBD:imm:Get"
--imm _i (GetItem _e1 _f _l) = error "TBD: imm GetItem"
imm _i (GetItem _e1 _f _l) = (i'', bs', mkId x _l)
  where
    (i' , bs, v1)	  = imm _i _e1
    (i'', x)		  = fresh _l i'
    bs'			  = (x, (GetItem v1 _f _l, _l)) : bs
--imm _i (App _e _es _l)     = error "TBD:imm:App"

imm _i (App _e _es _l)  = (_i'', bs', mkId x _l)
  where
    (_i' , bs, v:vs)    = imms _i (_e:_es)
    (_i'', x)           = fresh _l _i'
    bs'                 = (x, (App v vs _l, _l)) : bs
